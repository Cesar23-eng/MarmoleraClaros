...
export default function DetalleCotizacionModal({ cotizacion, clientes: clientesIniciales, onClose, onActualizada }: Props) {
  ...
  const puedeEditarEliminar = cotizacion.estado === 'Cotizado' && (role === 'Admin' || role === 'Ventas');
  const puedeAprobar        = cotizacion.estado === 'Cotizado' && (role === 'Admin' || role === 'Ventas');
  ...
  const [detalles, setDetalles]         = useState<DetalleCotizacionCreateDto[]>(
    cotizacion.detalles.map((d) => ({
      ...
      ladoA: 0, ladoB: 0, ladoC: undefined, ancho: undefined,
  ...
  const handleGuardar = async (e: React.FormEvent) => {
    ...
    const payload = {
      ClienteId: clienteId, Comentarios: comentarios || null,
      Detalles: detalles.map((d) => ({
        NombreMaterial: d.nombreMaterial, Geometria: d.geometria,
        LadoA: d.ladoA, LadoB: d.ladoB, LadoC: d.ladoC ?? null,
        Ancho: d.ancho ?? null, PrecioPorM2: d.precioPorM2,
      })),
    };
    try {
      setLoading(true);
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
        <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-6 space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-2">
            <button onClick={() => setModo('ver')} className="px-4 py-2 text-sm font-medium text-slate-600 hover:text-slate-800">Cancelar</button>
  ...
  if (modo === 'editar') {
    return (
      <>
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 overflow-y-auto py-8 px-4">
          <div className="w-full max-w-3xl bg-white rounded-2xl shadow-xl">
            ...
            <form onSubmit={handleGuardar} className="p-6 space-y-5">
              {error && <div className="bg-red-50 border border-red-100 text-red-600 rounded-lg px-4 py-3 text-sm">{error}</div>}
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Cliente *</label>
                  <div className="flex gap-2">
                    <select value={clienteId} onChange={(e) => setClienteId(Number(e.target.value))}
                      className="flex-1 px-3 py-2.5 text-sm border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                      <option value={0}>Seleccionar cliente...</option>
                      {clientes.map((c) => (<option key={c.id} value={c.id}>{c.nombreCompleto} — {c.telefono}</option>))}
                    </select>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <button type="button" onClick={() => setDetalles((p) => [...p, mesonVacio()])}
                    className="flex items-center gap-1.5 text-sm text-blue-600 hover:text-blue-800 font-medium">
                    <Plus size={15} /> Agregar mesón
                  </button>
                </div>
                {detalles.map((d, i) => (
                  <MesonForm key={i} index={i} detalle={d} onChange={handleChange}
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button type="button" onClick={() => setModo('ver')} className="px-4 py-2 text-sm font-medium text-slate-600 hover:text-slate-800">Cancelar</button>
        ...
        {modalCliente && (<NuevoClienteModal onClose={() => setModalCliente(false)} onCreado={handleClienteCreado} />)}
      </>
    );
  }
    ...
    <>
          ...
          <div className="p-6 space-y-5">
            <div className="grid grid-cols-2 gap-4 bg-slate-50 rounded-xl p-4">
              ...
              <div>
                ...
                <p className="font-semibold text-slate-800">{new Date(cotizacion.fechaCreacion).toLocaleDateString('es-BO')}</p>
                {cotizacion.comentarios && (<p className="text-sm text-slate-500 mt-1">{cotizacion.comentarios}</p>)}
              </div>
            </div>
            <div>
            ...
            <div className="flex items-center justify-between pt-1">
              <div className="flex gap-2">
                {puedeEditarEliminar && (
                  <>
                    ...
                    <button onClick={() => setModo('confirmarEliminar')}
              ...
              <div className="flex gap-2">
                <button onClick={onClose} className="px-4 py-2 text-sm font-medium text-slate-600 hover:text-slate-800">Cerrar</button>
                {puedeAprobar && (
                  <button onClick={() => setModalAprobar(true)} disabled={loading}
      ...
      {modalAprobar && (
        <AprobarCotizacionModal
          ...
          onAprobada={() => { setModalAprobar(false); onActualizada(); }}
        />
      )}
    </>
  );
